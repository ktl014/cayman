...
def convertDate(startTime, endTime):
    ...
    utcStart = calendar.timegm (pytz.timezone ('America/Los_Angeles').localize (
        datetime.datetime.strptime (startTime, '%Y-%m-%d %H:%M:%S')).utctimetuple ())
    utcEnd = calendar.timegm (pytz.timezone ('America/Los_Angeles').localize (
        datetime.datetime.strptime (endTime, '%Y-%m-%d %H:%M:%S')).utctimetuple ())
...
def getImages(urlList, outdir):
    ...
    with open ("poecilostomatoid_image_path_labels.csv", "w") as label_csv:
        ...
        for item in urlList:
            print
            'Starting download ' + item  # TODO:changed by Will
            urlToOpen = item
            ...
            nextPage = 'empty'
            print
            "Done with " + str (flag) + ' of ' + str (len (urlList))
            flag += 1
